...
# training parameters
filter_num = [96, 48, 24, 12] # number of conv filter, five numbers from big to small
feature_vector_dim = (3, 3, 1) # dimension of latent variables
batch_size = 20 # batch size
lr = 0.0002 # learning rate
train_epoch = int(15000) # # of training iterations
img_dim = (96,96,1) # dimension of input image
height, width = img_dim[0], img_dim[1]
D_steps = 1 # updating number of discriminator in one training iteration
...
def generator(x, isTrain=True, reuse=False):
    with tf.variable_scope('generator', reuse=reuse):
        # 1st hidden layer
        conv1 = tf.layers.conv2d_transpose(x, filter_num[0], [3, 3], strides=(2, 2), padding='same', name='conv1')
        relu1 = tf.nn.relu(tf.layers.batch_normalization(conv1, training=isTrain), name='relu1')

        # 2nd hidden layer
        conv2 = tf.layers.conv2d_transpose(relu1, filter_num[1], [3, 3], strides=(2, 2), padding='same', name='conv2')
        relu2 = tf.nn.relu(tf.layers.batch_normalization(conv2, training=isTrain), name='relu2')

        # 3rd hidden layer
        conv3 = tf.layers.conv2d_transpose(relu2, filter_num[2], [3, 3], strides=(2, 2), padding='same', name='conv3')
        relu3 = tf.nn.relu(tf.layers.batch_normalization(conv3, training=isTrain), name='relu3')

        # 4th hidden layer
        conv4 = tf.layers.conv2d_transpose(relu3, filter_num[3], [3, 3], strides=(2, 2), padding='same', name='conv4')
        ...
        conv6 = tf.layers.conv2d_transpose(relu4, 1, [3, 3], strides=(2, 2), padding='same', name='conv6')
...
def discriminator(x, isTrain=True, reuse=False):
    with tf.variable_scope('discriminator', reuse=reuse):
        ...
        conv1 = tf.layers.conv2d(x, filter_num[3], [3, 3], strides=(2, 2), padding='same', name='conv1')
        lrelu1 = lrelu(tf.layers.batch_normalization(conv1, training=isTrain, name='bn1'), 0.2)

        # 2nd hidden layer
        conv2 = tf.layers.conv2d(lrelu1, filter_num[2], [3, 3], strides=(2, 2), padding='same', name='conv2')
        lrelu2 = lrelu(tf.layers.batch_normalization(conv2, training=isTrain, name='bn2'), 0.2)

        # 3rd hidden layer
        conv3 = tf.layers.conv2d(lrelu2, filter_num[1], [3, 3], strides=(2, 2), padding='same', name='conv3')
        lrelu3 = lrelu(tf.layers.batch_normalization(conv3, training=isTrain, name='bn3'), 0.2)

        # 4th hidden layer
        conv4 = tf.layers.conv2d(lrelu3, filter_num[0], [3, 3], strides=(2, 2), padding='same', name='conv4')
        lrelu4 = lrelu(tf.layers.batch_normalization(conv4, training=isTrain, name='bn4'), 0.2)

        # output layer
        conv5 = tf.layers.conv2d(lrelu4, 1, [6, 6], strides=(1, 1), padding='valid', name='conv5')
...
def vgg_layers(x):
    ...
    for i in range(18):
        if (len(f[ks[i]].values())) != 0:        
            vgg16_weights.append(list(f[ks[i]].values())[0][:])
            vgg16_bias.append(list(f[ks[i]].values())[1][:])
        else:
            continue
    del f
...
#load our data
img_collection=[]
with open('Input_geometry_2000.pkl', 'rb') as f:
    img_collection = pickle.load(f,encoding='latin1')
img_collection = img_collection/255.0
...
for epoch in range(train_epoch):
    ...
    for G_i in range(G_steps):
        ...
        date=style_image_input
        image, loss_g_, _ = sess.run([G_z, G_loss, G_optim], {z: z_, x: x_, style_image:style_image_input, isTrain: True})
        G_losses.append(loss_g_)
        if epoch %500 ==0:
            plt.figure(0)
            plt.axis('off')
            plt.imshow(style_image_input[0])
    ...
    if epoch %100 ==0:
        ...
        plt.imshow(image[0])
        gray=image[0, :, :, 0]
        val = filters.threshold_otsu(image[0])
        #val = util.invert(val)
        plt.axis('off')
        plt.imshow(gray<val, cmap='gray')
        plt.savefig('./output/output_%s.jpg'%(epoch))
    ...
    if epoch %500==0:
        ...
        saver.save(sess, dir_name+'/model_%s'%(epoch), global_step=train_epoch-1)
        print("checkpoint at 500th epoch saved")
end_time = time.time()
